...
SYSTEM_CONFIG_PATH = "/etc/electrum.khc.conf"
...
class SimpleConfig(PrintError):
    ...
    def get_wallet_path(self):
        ...
        old_path = os.path.join(self.path, "electrum.khc.dat")
...
def read_system_config(path=SYSTEM_CONFIG_PATH):
    """Parse and return the system config settings in /etc/electrum.khc.conf."""
    result = {}
    if os.path.exists(path):
        try:
            import ConfigParser
        except ImportError:
            print "cannot parse electrum.khc.conf. please install ConfigParser"
            return
...
def read_user_config(path):
    """Parse and store the user config settings in electrum.khc.conf into user_config[]."""
    if not path:
        return {}
